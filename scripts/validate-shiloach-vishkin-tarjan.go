...
func ValidateShiloachVishkinTarjan(args []string) {
	...
	for _, graphSize := range graphSizesToTest {
		...
		for i := 0; i < iterationsPerGraphSize; i++ {
			...
			for _, threadsCount := range threadsCountsToTest {
				...
				_, blocksShiloachVishkinTarjan := graph.ShiloachVishkinTarjan(threadsCount)
				sortSlicesOfBlocks(blocksShiloachVishkinTarjan)

				if compareSlicesOfBlocks(blocksDFSTarjan, blocksShiloachVishkinTarjan) == 0 {
					rightAnswers++
				}
